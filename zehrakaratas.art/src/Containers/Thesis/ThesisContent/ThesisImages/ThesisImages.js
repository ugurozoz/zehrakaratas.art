  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width: `calc( 100% / ${imageList.length})`, left: `${currentImage !== 0 ? currentImage * (100 / imageList.length) : 0}%` }}
      >
        <span onClick={() => slideToImage(currentImage - 1)}>previous</span>
        <span onClick={() => slideToImage(currentImage + 1)}>next</span>
      </div>
    </div>
  );
};
